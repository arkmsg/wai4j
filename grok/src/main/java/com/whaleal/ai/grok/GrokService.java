...
public class GrokService implements AiService, AiServiceProvider, AiServiceEx {
    ...
    public String chat(AiConfig config, List<Message> messages) {
        try {
            ...
            if (!response.isSuccessful() || response.body() == null) {
                throw new AiException("Grok API error: " + response.code());
            }
            
            ChatCompletionResponse body = response.body();
            if (body.getChoices() == null || body.getChoices().isEmpty()) {
                throw new AiException("Empty response from Grok");
            }
            ...
            throw new AiException("Grok chat error: " + e.getMessage(), e);
        }
    }
    ...
    public Flux<String> chatStream(AiConfig config, List<Message> messages) {
        try {
            ...
            return Flux.error(new AiException("Grok stream chat error: " + e.getMessage(), e));
        }
    }

    private GrokApi createApi(AiConfig config) {
        ...
        return retrofit.create(GrokApi.class);
    }
